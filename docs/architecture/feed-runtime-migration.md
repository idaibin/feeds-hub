...
- Normal operator commands verify pooled `DATABASE_URL` with fixed role `feeds_app_runtime` and direct `DATABASE_URL_UNPOOLED` with the distinct reviewed migration owner; both resolve to the same Neon endpoint/database and the fingerprint covers both roles. During the explicitly enabled one-time Vercel Production bootstrap, the input pooled/direct aliases use the same owner; bootstrap derives the runtime URL, then invokes the normal runners with the required two-role split. After bootstrap, owner aliases are removed and runtime retains only `FEED_RUNTIME_DATABASE_URL`. No persistent URL is derived by hostname rewriting.
