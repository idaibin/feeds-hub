...
2. 固定节奏优先：已到窗口的 `stock`、活跃赛事、`github` 日更和 `hot` 小时覆盖。
   常规模式只跳过 `ai`，该 topic 由 Grok 独占发布；只有用户明确启动恢复模式且 Grok
   对应窗口已确认失败时，才可补一个 AI 缺口，不能自动并行覆盖。
...
- `github`：每天最多 1 条，优先昨天的高价值仓库或正式 release；来源延迟时跳过，不用当天数据代替。
- `lol`、`worldcup`：仅在 Active Event Calendar 的赛事窗口内按官方状态检查。
- `ai`：常规模式跳过；显式恢复模式优先过去 48 小时内的官方模型、产品、工具、研究或工程更新。
- 其它 topic 默认跳过。
