...
## 2026-07-13 Production Feed 内容初始化（已完成）
...
- 已执行 `CI=true npm run test`：最终 71/71 通过；`CI=true npm run test:mcp`：13/13 通过；`TEST_DATABASE_URL=... FEED_DB_TARGET=test CI=true npm run test:integration`：16/16 通过；`CI=true npm run test:e2e`：Content source 3 passed、database parity 1 skipped；`CI=true npm run db:generate`：无 schema changes；`CI=true npm run db:check`：通过，migration hash `38a653b70f50af03740f0d5edbc93f3fa5c4d3b2717148535a9976ece84b3b8c`；`CI=true npm run check`：83 files、0 errors、0 warnings、5 hints；`CI=true npm run build`：通过；`CI=true npm run content:import:dry`：234 insert、0 update、0 unchanged、0 conflict、0 invalid；`git diff --check`：通过。
- 首次 Production plan deployment `dpl_A4knN341AaaxXdeJ6E48mcVhYJTM` 在只读 schema preflight 失败，错误为 runtime 角色通过 `information_schema` 看不到按设计未授权的 `feed_import_runs`；未进入 import SQL、未写入 Feed，alias 保持上一条 READY deployment。修复改用固定 `pg_catalog` / `to_regclass` 对象清单验证，不授予 runtime 访问 `feed_import_runs`。
- 代码最终通过任务分支 `feat/feed-production-content-import` 交付；功能 commit `4b77f29bdef196f4180327a13e0fc2b08174dbec`、schema preflight 修复 commit `829b634a9acc6ff1f189e36ee1c5e25a494d4af0` 均已 push。对应 main commits 为 `5bd695444f640fe97d3d4f9abd8d39b9a55aac80` 与 `631481b29343b2c221fe7efbaf7f37d5c2633e05`；未创建 PR，本地分叉 `main` 未切换或改写。
- 修复后的只读 plan deployment `dpl_DMHhWdCFxUGWNUzqjeN26A74YuDG` 为 READY，source commit `631481b29343b2c221fe7efbaf7f37d5c2633e05`；database plan 为 234 insert、0 update、0 unchanged、0 conflict、0 invalid，source tree hash `b254639ae09d6e3768da397f230f38c61c2160e96f93893d962b583ce8ec91fa`。
- 导入前创建 Neon recovery branch `backup-before-feed-import-20260713`（`br-red-hall-atuq9380`），parent 为 Production `main`（`br-raspy-rice-at72v136`），创建于 `2026-07-13 11:01:18 +08:00`，自动保留至 `2026-07-14 11:01 +08:00`；recovery entry 为该 branch 的 Neon Console 页面。
